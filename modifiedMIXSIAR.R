...
#' @export
#'   
output_JAGS2 <- function(jags.1, mix, source, output_options=list(
  ...
  if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
    ...
    if(mix$n.fe == 0){ # only if there are no fixed effects, otherwise p.global is meaningless
      ...
      plots <- lapply(unique(df$sources), function(source) {
        ggplot(subset(df, sources == source), aes(x = x)) +
          geom_density(color = "black", fill = "gray80", alpha = 0.5, aes(y = ..scaled..)) +
          theme_bw() +
